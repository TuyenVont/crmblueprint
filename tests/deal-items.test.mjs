...
test('DEALS_MANAGE is required for create/update/delete', async () => {
  for (const p of [[], ['DEALS_VIEW']]) {
    ...
    await assert.rejects(s.updateDealItem(dealItemId, itemForm()), /NOT_FOUND/)
    await assert.rejects(s.deleteDealItem(dealItemId), /NOT_FOUND/)
  }
})
...
test('foreign Deal Item update and delete are rejected', async () => {
  ...
  const resUpdate = await sUpdate.updateDealItem(dealItemId, itemForm())
  ...
  const resDelete = await sDelete.deleteDealItem(dealItemId)
...
test('later Product changes do not alter Deal Item snapshot behavior', async () => {
  ...
  await s.updateDealItem(dealItemId, form)
...
test('total is generated by database and not writable', async () => {
  ...
  await sEdit.updateDealItem(dealItemId, form)
...
test('discount <= price * quantity validation rejects excessive discount', () => {
  const f = itemForm()
  f.set('price', '100')
  f.set('quantity', '2')
  f.set('discount', '300') // subtotal is 200
  const res = validation.validateDealItemInput(f, true)
